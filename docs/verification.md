...
node --check src/rigged-spider.js
...
coxa shell placement, front-foot forwardness, per-pair stepping, and rig
endpoint error.
...
3. `M` switches one model off before showing the other.
4. `Space` triggers the pose without stopping rendering.
