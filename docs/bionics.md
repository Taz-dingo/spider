...
2. 桌宠跨屏失败根因修正（2026-08-21）：此前"多屏 union 不可达/服务器拒绝
   跨屏"的结论是**在一台具体设备上观察到的，不能当作通用规律**——窗口服务器
   对多屏 borderless 窗口的落位随排列而异，有的接受真实 union、有的会把它
   重定位到重叠最多的屏的原点。真正通用、可复现的缺陷在 `placeWindow` 的
   候选顺序：它把 `union@0,0`（锚定主屏原点 x=0 的 union）放在**真实 union**
   之前；因 `union@0,0` 也包含主屏中心而被优先选中，窗口相对真实 desktop
   union 被整体平移了（平移量 = 副屏的 x 悬挑），恰好漏掉副屏悬挑区，蜘蛛
   到不了那里的光标（停缝）。修复：**优先尝试真实 desktop union（true
   origin）**，且运行时只接受真正覆盖 desktop union 中心的窗口，回退到只覆盖
   主屏、并在最后一个候选强制接受（服务器彻底拒绝跨屏时仍留可用窗口）。
    该修复纯自适应，不依赖任何具体屏幕尺寸或排列。L3 集成测试把"窗口是否覆盖
    整个 desktop union 中心"作为诊断输出而非硬断言——跨屏覆盖取决于窗口服务器
    是否接受多屏 union，随排列而异，属于设备属性。残余：30° 倾斜下副屏顶部约
    90px 仍被 margin clamp 截断（非阻塞，蜘蛛可到达副屏绝大部分）。

3. 桌宠跟准 + 跨屏跟随修复（2026-08-22）：真正的根因是**窗口边缘投影**
   `petPointer`——光标落在任意桌面窗口内时，follow 目标会被拉硬到该窗口的
   最近边（`wx±14` 等），而不是光标本身。后果：光标在窗口内就"跟不准"（蜘蛛
   去窗框而非光标），副屏被窗口占满时目标一直被吸到窗框上，蜘蛛永远到不了
   副屏中心。修复：**彻底移除窗口投影**，follow 一律直接追光标真实坐标
   （`pointer.set(x,0,z)`），并随之移除窗框压扁（`petFlatten`）、移除 Swift 端
   每 10Hz 的 CGWindowList 窗口注入（不再需要）。同时把步态跟随即阈值从 25
   降到 12，蜘蛛贴脸到光标约 7px。实测（双屏，主 1470x956 + 副 1920x1080 上方
   悬挑）：无投影时蜘蛛从主屏中心爬到副屏中心 `(0,-558)`，误差仅 6.6 units；
   注入 `petMouse` 与 Swift 换算完全一致（delta=0）；窗口/coordinateFrame 覆盖
   全桌面。全测试 21 项绿。
