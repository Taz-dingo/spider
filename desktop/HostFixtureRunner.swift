...
//   mainScreenHeight: Double    for the CGWindowList y flip, optional
//   windows: [[x,y,w,h], ...]   display-coord rects, optional
...
struct HostFixtureRunner {
    static func main() throws {
        ...
        if let mouse = fixture["mouse"] as? [Double], mouse.count == 2 {
            let r = HostGeometry.mouseToPage(NSPoint(x: mouse[0], y: mouse[1]), frame: frame, viewZ: viewZ)
            out["mouseToPage"] = [r.x, r.z]
        }
