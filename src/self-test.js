...
function startSelfTest(name) {
  ...
  testRun = {
    ...
    elapsed: 0, phaseElapsed: 0, phase: 0, steps: 0, predictiveReplants: 0, maxReach: 0, maxSector: 0, maxTurn: 0, minFootGap: Infinity, maxLegCrossings: 0, maxCoxaShellError: 0, bodyPenetrations: 0, bodyTravel: 0, footTravel: 0, commandedBodyTravel: 0, supportCorrectionTravel: 0, hardSupportStopTime: 0, commandedBodyTurn: 0, turnCorrection: 0, hardTurnStopTime: 0, turnReplantTravel: 0, twitchTime: 0, maxStall: 0, stallTime: 0, timeouts: 0,
...
function updateSelfTest(delta) {
  ...
  // Normal support pressure may reduce travel/turning, but a full geometry-
  // driven body freeze should now be exceptional and too short to read as a
  // visible pause.
  const supportContinuityPass = testRun.hardSupportStopTime <= .18;
  const headingContinuityPass = testRun.hardTurnStopTime <= .18;
  ...
  const passed = complete && testRun.timeouts === 0 && testRun.steps >= 8 && testRun.maxReach <= 66 && testRun.maxSector <= .9 && testRun.minFootGap >= 9 && testRun.maxLegCrossings <= 2 && testRun.maxCoxaShellError < .001 && testRun.maxTurn >= testRun.minTurn && angleEnvelopePass && frontPass && turnContinuityPass && supportContinuityPass && headingContinuityPass;
  window.__spiderSelfTest = { name: testRun.name, running: !complete, passed: complete && passed, elapsed: testRun.elapsed, phase: testRun.phase, steps: testRun.steps, predictiveReplants: testRun.predictiveReplants, pairSteps: testRun.pairSteps, maxReach: testRun.maxReach, maxSector: testRun.maxSector, maxTurn: testRun.maxTurn, minFootGap: testRun.minFootGap, legCrossings: testRun.maxLegCrossings, maxCoxaShellError: testRun.maxCoxaShellError, bodyPenetrations: testRun.bodyPenetrations, bodyTravel: testRun.bodyTravel, footTravel: testRun.footTravel, commandedBodyTravel: testRun.commandedBodyTravel, supportCorrectionTravel: testRun.supportCorrectionTravel, hardSupportStopTime: testRun.hardSupportStopTime, supportContinuityPass, commandedBodyTurn: testRun.commandedBodyTurn, turnCorrection: testRun.turnCorrection, hardTurnStopTime: testRun.hardTurnStopTime, headingContinuityPass, turnReplantTravel: testRun.turnReplantTravel, turnContinuityPass, gaitEfficiency, twitchTime: testRun.twitchTime, maxStall: testRun.maxStall, crossingPairs: [...testRun.crossingPairs], femurPatella: testRun.femurPatella, distal: testRun.distal, terminal: testRun.terminal, frontTouchdown: testRun.frontTouchdown, frontPass, finishError: error, timeouts: testRun.timeouts, heading: spider.angle, turnBlocked: testRun.turnBlocked };
